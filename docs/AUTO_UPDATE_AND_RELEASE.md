...
WorkflowはPreview用と正式版用を分離する。

| Workflow | 起点 | バージョン | Release | 承認Environment |
|---|---|---|---|---|
| `release.yml` | `staging` | `X.Y.Z-preview.N` | prerelease | `preview-release` |
| `production-release.yml` | `main` | `X.Y.Z` | 正式Release | `production-release` |

正式版Workflowも手動実行時は`main`を選択した場合だけ進行する。Preview Releaseやdraftは正式版のpatch採番基準に含めない。
...
正式版は、検証済みの変更を`main`へ反映し、`production-release` Environmentで承認する。公開jobは最新の正式版からpatchを増やした`vX.Y.Z`タグを作り、prereleaseフラグなしで公開する。
