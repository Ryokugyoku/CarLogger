...
class InferenceEngine:
    ...
    def _candidates(self, scope: str) -> list[dict[str, Any]]:
        ...
        for metadata_path in sorted(
            models.glob("*/metadata.json"), key=lambda p: p.stat().st_mtime, reverse=True
        ):
    ...
    def load(self, feature_schema: str, scope: str = "global") -> dict[str, Any]:
        ...
        for index, candidate in enumerate(candidates):
            try:
                ...
                metadata = json.loads(
                    (artifact.parent / "metadata.json").read_text(encoding="utf-8")
                )
                ...
                self.model, self.metadata, self.model_id = (
                    model,
                    metadata,
    ...
    def infer(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "score": score if confidence >= 0.60 else None,
